...
import { IIGIdentification, IIGObservation } from 'ikimongo-types';
...
export class NoteEntityService implements OnModuleInit {
  ...
  constructor(
    ...
    private observationsRepository: IGObservationsRepository,

    @Inject(DI.identificationsRepository)
    private identificationsRepository: IGIdentificationsRepository, //private driveFileEntityService: DriveFileEntityService,
    //private userEntityService: UserEntityService,
  ) //private customEmojiService: CustomEmojiService,
  //private reactionService: ReactionService,
  {}
  ...
  private async hideNote(
    ...
    if (hide) {
      ...
      packedNote.observation = undefined;
      packedNote.identification = undefined;
      packedNote.cw = null;
  ...
  @bindThis
  private async populateObservation(note: MiNote): Promise<IIGObservation> {
    const observation = await this.observationsRepository.findOneBy({
      noteId: note.id,
    });

    const decimalLatitude = observation?.location?.coordinates[1];
    const decimalLongitude = observation?.location?.coordinates[0];
    const response = Object.assign({}, observation);
    delete (response as { location?: Point }).location;

    return {
      decimalLatitude,
      decimalLongitude,
      ...(response as IIGObservation),
    };
  }

  @bindThis
  private async populateIdentification(note: MiNote) {
    const identification = await this.identificationsRepository.findOneBy({
      noteId: note.id,
    });
    ...
    const packed: Packed<'Note'> = await awaitAll({
      ...
      ...(opts.detail
        ? {
            ...
            identification: note.hasIdentification
              ? this.populateIdentification(note)
              : undefined,
