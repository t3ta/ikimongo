...
import type { IMentionedRemoteUsers, MiNote } from "@/models/note/Note.js";
...
import type { MiPollVote } from "@/models/poll/PollVote.js";
...
export class ApRendererService {
	...
	public async renderNote(note: MiNote, dive = true): Promise<IPost> {
		const getPromisedFiles = async (ids: string[]): Promise<MiDriveFile[]> => {
			...
			return ids
				...
				.filter((item): item is MiDriveFile => item != null);
		};
		...
		if (note.replyId) {
			...
			if (inReplyToNote != null) {
				const inReplyToUserExist = await this.usersRepository.exist({
	...
	public renderUndo(
		...
		const id =
			...
			object.id.startsWith(this.config.url)
				? `${object.id}/undo`
				: undefined;
