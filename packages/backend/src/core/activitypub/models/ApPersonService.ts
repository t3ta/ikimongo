...
import type { MiLocalUser, MiRemoteUser } from "@/models/user/User.js";
...
export class ApPersonService implements OnModuleInit {
	...
	public async createPerson(
		...
		const emojis = await this.apNoteService
			...
			.catch((err) => {
				this.logger.error("error occurred while fetching user emojis", {
					stack: err,
				});
				return [];
			});
		...
		try {
			...
			this.logger.error("error occurred while fetching user avatar/banner", {
				stack: err,
			});
		}
		//#endregion
