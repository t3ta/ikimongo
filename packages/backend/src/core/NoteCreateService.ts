...
const mutedWordsCache = new MemorySingleCache<
  { userId: MiUserProfile['userId']; mutedWords: MiUserProfile['mutedWords'] }[]
>(1000 * 60 * 5);
...
export class NoteCreateService implements OnApplicationShutdown {
  ...
  public async create(
    ...
    if (data.channel) {
      this.redisClient.xadd(
        ...
        'MAXLEN',
        '~',
        this.config.perChannelMaxNoteCacheCount.toString(),
        '*',
        'note',
        note.id,
      );
  ...
  private async insertNote(
    ...
    // 投稿を作成
    try {
      if (insert.hasPoll || insert.hasObservation || insert.hasIdentification) {
        // Start transaction
        await this.db.transaction(async (transactionalEntityManager) => {
          ...
          if (insert.hasObservation) {
            observationId = this.idService.genId(data.createdAt!);
            ...
            const observation = new IGObservation({
              ...
              location: {
                type: 'Point',
                coordinates: [
                  data.observation!.decimalLatitude!,
                  data.observation!.decimalLongitude!,
                ],
              },
            });
          ...
          if (insert.hasIdentification) {
            if (observationId == null) {
              const observation =
                await this.observationsRepository.findOneByOrFail({
                  noteId: insert.replyId!,
                });
