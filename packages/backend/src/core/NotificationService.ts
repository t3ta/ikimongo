...
import type { MiNotification } from "@/models/note/Notification.js";
	...
	public async createNotification(
		...
		if (notifierId) {
			...
			if (recieveConfig?.type === "following") {
				...
				const isMember = await this.userListService.membersCache
			...
			async () => {
				...
				if (type === "follow")
					this.emailNotificationFollow(
						notifieeId,
						await this.usersRepository.findOneByOrFail({ id: notifierId! }),
					);
