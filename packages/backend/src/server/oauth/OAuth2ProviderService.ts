...
function validateClientId(raw: string): URL {
	...
	const allowedProtocols =
		process.env.NODE_ENV === "test" ? ["http:", "https:"] : ["https:"];
...
async function discoverClientInformation(
	...
	try {
		...
		redirectUris.push(
			...[
				...fragment.querySelectorAll<HTMLLinkElement>(
		...
		if (text) {
			...
			const nameProperty = microformats.items.find(
				(item) =>
					item.type?.includes("h-app") && item.properties.url.includes(id),
			)?.properties.name[0];
...
class OAuth2Store {
	...
	load(
		req: OAuth2DecisionRequest,
		cb: (err: Error | null, txn?: OAuth2) => void,
	): void {
	...
	store(
		req: OAuth2DecisionRequest,
		oauth2: OAuth2,
		cb: (err: Error | null, transactionID?: string) => void,
	): void {
...
export class OAuth2ProviderService {
			...
			oauth2orize.grant.code(
				...
				(client, redirectUri, token, ares, areq, locals, done) => {
					(async (): Promise<OmitFirstElement<Parameters<typeof done>>> => {
						...
						const user =
							await this.cacheService.localUserByNativeTokenCache.fetch(
								token,
								() =>
		...
		this.#server.exchange(
			oauth2orize.exchange.authorizationCode(
				(client, code, redirectUri, body, authInfo, done) => {
					(async (): Promise<
						OmitFirstElement<Parameters<typeof done>> | undefined
					> => {
						this.#logger.info(
	...
	public async createServer(fastify: FastifyInstance): Promise<void> {
		...
		fastify.use(
			"/oauth/authorize",
			this.#server.authorize(((areq, done) => {
				(async (): Promise<Parameters<typeof done>> => {
					...
					const clientInfo = await discoverClientInformation(
						this.#logger,
