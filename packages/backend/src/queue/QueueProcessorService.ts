...
export class QueueProcessorService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.systemQueueWorker
			...
			.on("failed", (job, err) =>
				systemLogger.warn(`failed(${err}) id=${job ? job.id : "-"}`, {
					job,
					e: renderError(err),
				}),
			)
			.on("error", (err: Error) =>
				systemLogger.error(`error ${err}`, { e: renderError(err) }),
			)
		...
		this.dbQueueWorker
			...
			.on("failed", (job, err) =>
				dbLogger.warn(`failed(${err}) id=${job ? job.id : "-"}`, {
					job,
					e: renderError(err),
				}),
			)
			.on("error", (err: Error) =>
				dbLogger.error(`error ${err}`, { e: renderError(err) }),
			)
		...
		this.deliverQueueWorker
			...
			.on("failed", (job, err) =>
				deliverLogger.warn(
					`failed(${err}) ${getJobInfo(job)} to=${job ? job.data.to : "-"}`,
				),
			)
			.on("error", (err: Error) =>
				deliverLogger.error(`error ${err}`, { e: renderError(err) }),
			)
			...
			.on("failed", (job, err) =>
				inboxLogger.warn(
					`failed(${err}) ${getJobInfo(job)} activity=${
						job ? (job.data.activity ? job.data.activity.id : "none") : "-"
					}`,
					{ job, e: renderError(err) },
				),
			)
			.on("error", (err: Error) =>
				inboxLogger.error(`error ${err}`, { e: renderError(err) }),
			)
			.on("stalled", (jobId) => inboxLogger.warn(`stalled id=${jobId}`));
		//#endregion

		//#region webhook deliver
		this.webhookDeliverQueueWorker = new Bull.Worker(
			QUEUE.WEBHOOK_DELIVER,
			(job) => this.webhookDeliverProcessorService.process(job),
			{
		...
		this.webhookDeliverQueueWorker
			...
			.on("error", (err: Error) =>
				webhookLogger.error(`error ${err}`, { e: renderError(err) }),
			)
		...
		this.relationshipQueueWorker
			...
			.on("failed", (job, err) =>
				relationshipLogger.warn(`failed(${err}) id=${job ? job.id : "-"}`, {
					job,
					e: renderError(err),
				}),
			)
			.on("error", (err: Error) =>
				relationshipLogger.error(`error ${err}`, { e: renderError(err) }),
			)
		...
		this.objectStorageQueueWorker
			...
			.on("failed", (job, err) =>
				objectStorageLogger.warn(`failed(${err}) id=${job ? job.id : "-"}`, {
					job,
					e: renderError(err),
				}),
			)
			.on("error", (err: Error) =>
				objectStorageLogger.error(`error ${err}`, { e: renderError(err) }),
			)
			.on("stalled", (jobId) =>
		...
		this.endedPollNotificationQueueWorker = new Bull.Worker(
			...
			(job) => this.endedPollNotificationProcessorService.process(job),
			{
