...
async function getClientWidthWithCache(
	targetEl: HTMLElement,
	containerEl: HTMLElement,
	count = 0,
) {
	if (_DEV_)
		console.log("getClientWidthWithCache", {
			targetEl,
			containerEl,
			count,
			cache: widthCache.get(containerEl),
		});
...
import XBanner from "@/components/mk_components//MkMediaBanner.vue";
import XImage from "@/components/mk_components//MkMediaImage.vue";
import XVideo from "@/components/mk_components//MkMediaVideo.vue";
...
async function calcAspectRatio() {
	...
	const width = container.value
		? await getClientWidthWithCache(root.value, container.value)
	...
	const heightMin = (ratio: number) => {
		...
		if (_DEV_)
			console.log("Image height calculated:", {
				width,
		...
		default: {
			const maxHeight = Math.max(
				...
				(container.value
					...
					: getBodyScrollHeight()) * 0.5 || 360,
			);
			if (width === 0 || !maxHeight) return;
			const imgResizeRatio = width / img.properties.width;
			const imgDrawHeight = img.properties.height * imgResizeRatio;
			gallery.value.style.height = `${Math.max(
				64,
				Math.min(imgDrawHeight, maxHeight),
			)}px`;
