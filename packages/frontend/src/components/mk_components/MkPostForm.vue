  ...
  >
    ...
    <section class="container">
      ...
      <IGObservationEditor
        v-if="observation"
        v-model="observation"
        @destroyed="observation = null"
      />
      <IGIdentificationEditor
        v-if="identification"
        v-model="identification"
        @destroyed="identification = null"
      />
      <input
        v-show="useCw"
        ref="cwInputEl"
...
import {
  IIGObservationRequest,
  IIGIdentificationRequest,
} from 'ikimongo-types';
import IGObservationEditor from '@/components/ig_components/IGObservationEditor.vue';
import IGIdentificationEditor from '@/components/ig_components/IGIdentificationEditor.vue';
import MkNoteSimple from '@/components/mk_components/MkNoteSimple.vue';
import MkNotePreview from '@/components/mk_components/MkNotePreview.vue';
import XPostFormAttaches from '@/components/mk_components/MkPostFormAttaches.vue';
import MkPollEditor from '@/components/mk_components/MkPollEditor.vue';
...
import MkRippleEffect from '@/components/mk_components/MkRippleEffect.vue';
...
let identification = $ref<IIGIdentificationRequest | null>(null);
// eslint-disable-next-line no-undef
let observation = $ref<IIGObservationRequest | null>(null);
let useCw = $ref(false);
...
function setVisibility() {
  ...
  os.popup(
    defineAsyncComponent(
      () => import('@/components/mk_components/MkVisibilityPicker.vue'),
    ),
    {
...
async function onPaste(ev: ClipboardEvent) {
  for (const { item, i } of Array.from(ev.clipboardData.items, (item, i) => ({
    item,
    i,
  }))) {
    if (item.kind === 'file') {
...
async function post(ev?: MouseEvent) {
  ...
  posting = true;
  os.api(props.updateMode ? 'notes/update' : 'notes/create', postData, token)
    .then(() => {
