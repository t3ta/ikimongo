			...
			<template v-for="(item, i) in items2">
				...
				<button
					...
					:class="[
						$style.item,
						$style.switch,
						{ [$style.switchDisabled]: item.disabled },
					]"
				...
				<button
					...
					@mouseenter="preferClick ? null : showChildren(item, $event)"
					@click="!preferClick ? null : showChildren(item, $event)"
				>
					<i
						v-if="item.icon"
						class="ti-fw"
...
import MkSwitchButton from "@/components/mk_components/MkSwitch.button.vue";
import {
	MenuItem,
	InnerMenuItem,
	MenuPending,
