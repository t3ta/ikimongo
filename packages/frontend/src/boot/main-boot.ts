	...
	const { isClientUpdated } = await common(() =>
		createApp(
			new URLSearchParams(window.location.search).has("zen") ||
				(ui === "deck" &&
					deckStore.state.useSimpleUiForNonRootPages &&
					location.pathname !== "/")
	...
	if (isClientUpdated && $i) {
		popup(
			defineAsyncComponent(
				() => import("@/components/mk_components/MkUpdated.vue"),
			),
			{},
			{},
			"closed",
	...
	if ($i) {
		...
		defaultStore.loaded.then(() => {
			if (defaultStore.state.accountSetupWizard !== -1) {
				popup(
					defineAsyncComponent(
						() => import("@/components/mk_components/MkUserSetupDialog.vue"),
					),
					{},
					{},
					"closed",
		...
		for (const announcement of ($i.unreadAnnouncements ?? []).filter(
			...
			popup(
				defineAsyncComponent(
					() => import("@/components/mk_components/MkAnnouncementDialog.vue"),
				),
				{
					announcement,
				},
		...
		if (
			...
			if (
				...
				popup(
					defineAsyncComponent(
						() => import("@/components/mk_components/MkDonation.vue"),
					),
					{},
					{},
					"closed",
