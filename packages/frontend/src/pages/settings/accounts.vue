			...
			<div class="_gaps">
				<div class="_buttons">
					...
					<MkButton @click="init"
						...
						{{ i18n.ts.reloadAccountsList }}</MkButton
					>
				</div>
...
function addExistingAccount() {
	os.popup(
		defineAsyncComponent(
			() => import("@/components/mk_components/MkSigninDialog.vue"),
		),
		{},
		{
			done: async (res) => {
...
function createAccount() {
	os.popup(
		defineAsyncComponent(
			() => import("@/components/mk_components/MkSignupDialog.vue"),
		),
		{},
		{
			done: async (res) => {
